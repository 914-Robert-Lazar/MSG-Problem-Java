...
public class SavingsManagerService {
    ...
    public void passTime() {
        ...
        LocalDate nextSystemDateForQuarterly = systemDate.plusMonths(3);
        ...
        savingAccounts.forEach(savingAccount -> {
            if (savingAccount.getInterestFrequency() == CapitalizationFrequency.MONTHLY) {
                ...
                addMonthlyInterest(savingAccount, nextSystemDateForQuarterly);
            }
        });
    ...
    private void addMonthlyInterest(SavingsAccountModel savingAccount, LocalDate currentInterestMonth) {
        LocalDate nextInterestDateForAccount;
        if (savingAccount.getInterestFrequency() == CapitalizationFrequency.MONTHLY) {
            nextInterestDateForAccount = savingAccount.getLastInterestAppliedDate().plusMonths(1);
        }
        else {
            nextInterestDateForAccount = savingAccount.getLastInterestAppliedDate().plusMonths(3);
        }
